...
%_______________________
\subsection{\ttfamily Locat\normalfont-ing Observations}
%%%%%%% New frame
\begin{frame}[fragile, allowframebreaks]
\frametitle{\ttfamily Locat\normalfont-ing Observations}
\framesubtitle{Preliminaries}

To determine a line though $n$ observations, use \ttfamily locator(): \normalfont
...
%\newpage
%\begin{center}
%\includegraphics[scale=0.25]{images/xy.png}
%\end{center}
...
\begin{frame}[fragile, allowframebreaks]
\frametitle{\ttfamily Locat\normalfont-ing Observations}
\textbf{Left-click} \normalfont on observations to be the two endpoints of the line.
\noindent \textbf{Right-click} \normalfont on the observation to exit the function.
	\begin{lstlisting}
# Estimate the slope and intercept of line:	
a<-locator(n = 12, type = "l", col="red")
m=( a$y[2]- a$y[1])/( a$x[2]- a$x[1]); m	# slope
b=a$y[1]-m*a$x[1]; b					# intercept
abline(a=b, b=m, col=ÓblueÓ)
	\end{lstlisting}

\newpage
\begin{center}
\includegraphics[scale=0.27]{images/xyLoc.png}
\end{center}
...
To find out out the observation number from the plot, use \ttfamily identify(): \normalfont
	\begin{lstlisting}
		# Generate data and fit a regression curve:
		set.seed(3012008)
		x=rnorm(100); y=-x+I(x^2) +rnorm(100)
